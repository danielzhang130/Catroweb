  ...
  private function validateLanguage(?string $language): void
  {
    if (2 == strlen($language)) {
      if (strtolower($language) != $language) {
        throw new InvalidArgumentException("2-character language code {$language} has to be lower case");
      }

      if (!Languages::exists($language)) {
        throw new InvalidArgumentException("2-character language code {$language} is invalid");
      }
    } elseif (5 == strlen($language)) {
      if ('-' != $language[2]) {
        throw new InvalidArgumentException("Invalid 5-character language code format: {$language}");
      }
      ...
      if (strtolower($language_code) != $language_code) {
        throw new InvalidArgumentException("5-character language code {$language} has to contain lower case language code");
      }

      if (!Languages::exists($language_code)) {
        throw new InvalidArgumentException("language code {$language_code} in 5-character language code is invalid");
      }

      if (strtoupper($country_code) != $country_code) {
        throw new InvalidArgumentException("5-character language code {$language} has to contain upper case country code");
      }

      if (!Countries::exists($country_code)) {
        throw new InvalidArgumentException("country code {$country_code} in 5-character language code is invalid");
      }
